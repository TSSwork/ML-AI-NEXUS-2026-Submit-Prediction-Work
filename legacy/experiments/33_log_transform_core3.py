"""
NEW — tests the "Log-Normal Physiological Transformation" suggestion:
creatinine_mg_dl and length_of_stay_days are right-skewed clinical
variables; log-transforming them may help the Spline GAM and CatBoost
models (both use these as raw continuous inputs). Notably, length_of_stay
also showed up as a top-5 driver in the adversarial-validation feature
importances (32_adversarial_validation.py) -- compressing its tail could
plausibly help with the shift too, not just the marginal distribution.

Adds log_creatinine = ln(creatinine_mg_dl) and log_los =
ln(length_of_stay_days + 1) alongside (not replacing) the raw columns,
then retrains the honest Core-3 (Spline GAM + EBM + CatBoost, no
LightGBM, no phenotype-cluster features) 3-seed-bagged, exactly as in
27_final_recommended_stack.py / the notebook's new cells, and compares
against the 0.34855 honest baseline.
"""
import pickle
...
# ---- log-transform the two right-skewed clinical variables ----
for df in [train_fe, test_fe]:
    df['log_creatinine'] = np.log(df['creatinine_mg_dl'])
    df['log_los'] = np.log(df['length_of_stay_days'] + 1)
...
# spline_cols now includes the two log features alongside the raw ones,
# so the spline basis sees both the raw and log-compressed versions
spline_cols = [
...
for seed in SEEDS:
    ...
    for trn_idx, val_idx in skf_seed.split(X, y):
        ...
        ebm.fit(X_tr_raw[ebm_cols], y_tr)
        ...
        cb.fit(X_tr_cb, y_tr, cat_features=cat_features, eval_set=(X_va_cb, y_va), early_stopping_rounds=30)
...
print(f"\nSpline GAM (log-transform, 3-seed) OOF Log Loss: {log_loss(y, oof_spline):.5f}  (no-log baseline: 0.34991)")
print(f"EBM (log-transform, 3-seed) OOF Log Loss:        {log_loss(y, oof_ebm):.5f}  (no-log baseline: 0.34928)")
print(f"CatBoost (log-transform, 3-seed) OOF Log Loss:    {log_loss(y, oof_cb):.5f}  (no-log baseline: 0.34965)")
...
print("\n" + "=" * 62)
print("  CORE-3 + LOG-TRANSFORMED CREATININE/LOS -- RESULT")
print("=" * 62)
...
print(f"Reference -- Core-3 WITHOUT log transform (honest): 0.34855")
...
out_path = ROOT / "legacy" / "submissions" / "submission_14_core3_log_transform.csv"
